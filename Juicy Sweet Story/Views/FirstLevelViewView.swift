...
struct FirstLevelViewView: View {
    ...
    @State var bestTime = UserDefaults.standard.integer(forKey: UserDefaultsKey.bestTime)
        ...
        ZStack {
            ...
            RoundedRectangle(cornerRadius: 60)
                ...
                .overlay {
                            ...
                            Image(String(puzzle))
                                ...
                                .onChange(of: vm.randomPuzzles) { newValue in
                                    if vm.randomPuzzles == solvedPuzzles {
                                        ...
                                        if vm.openLevels?.count == 1 {
                                            UserDefaults.standard.set([0,1], forKey: UserDefaultsKey.openLevels)
                                            vm.openLevels = UserDefaults.standard.array(forKey: UserDefaultsKey.openLevels)
                                        }

                                        gameTime = 180 - vm.timeRemaining
                                        if gameTime < bestTime {
                                            UserDefaults.standard.set(gameTime, forKey: UserDefaultsKey.bestTime)
                                            bestTime = gameTime
                                        }
                                        if bestTime == 0 {
                                            bestTime = gameTime
                    ...
                    .clipShape(CustomCorners(corners: [.bottomLeft, .bottomRight], radius: 56))
